...
__all__ = ["LoadImaged", "LoadImageD", "LoadImageDict", "LazyLoadImaged", "LazyLoadImageD", "LazyLoadImageDict", "SaveImaged", "SaveImageD", "SaveImageDict"]
...
class LazyLoadImaged(MapTransform):
    def __init__(
        self,
        keys: KeysCollection,
        roi_key: KeysCollection,
        reader: type[ImageReader] | str | None = None,
        dtype: DtypeLike = np.float32,
        meta_keys: KeysCollection | None = None,
        meta_key_postfix: str = DEFAULT_POST_FIX,
    ...
    def __call__(self, data, roi_slices: Sequence[slice] | None = None, reader: ImageReader | None = None):
        """
        Raises:
            KeyError: When not ``self.overwriting`` and key already exists in ``data``.
        ...
        d = dict(data)
        for key, meta_key, meta_key_postfix in self.key_iterator(d, self.meta_keys, self.meta_key_postfix):
            data = self._loader(d[key], roi_slices or d[self.roi_key], reader)
            if self._loader.image_only:
                d[key] = data
            else:
                if not isinstance(data, (tuple, list)):
